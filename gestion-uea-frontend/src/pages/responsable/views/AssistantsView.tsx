...
const AssistantsView = () => {
  ...
  const fetchData = async () => {
    try {
      const data = await apiClient('/users?role=assistant');
      setAssistants(data);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAssistants(prev => [...prev, response.user]);
      setForm({ name: '', email: '', specialite: '' });
      setSuccess('Assistant ajouté avec succès');
  ...
  return (
    <div>
      {mode === 'form' ? (
        ...
        <>
          <h3>Liste des assistants techniques ({assistants.length})</h3>
            ...
            <tbody>
              {assistants.map(a => (
                <tr key={a.id}>
                  ...
                  <td style={tdStyle}>{a.specialite || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
